...
<?php
   ...
   $sql1="SELECT Name,Department,Designation,CollegeName,Imagepath FROM Staff Where IDNo='$emp_id'";
...
<div class="card-body table-responsive p-0" style="height: 100%;">
   ...
   <select class="form-control" name="" id='locationID'  >
      ...
      <div class="col-lg-3">
         <label>&nbsp;</label>
         <button type="submit" data-dismiss="modal" class="form-control btn-danger btn" onclick="DiscardSubmit(<?=$id?>)">Submit</button>
      </div>
   </div>
               <?php 
            }
...
<div class="card card-info">
   ...
   <div class="card-body table-responsive p-0" style="height: 1000px;">
      <?php
         $emp_id = $EmployeeID;
         $sql1 = "SELECT Name,Department,Designation,CollegeName,Imagepath FROM Staff Where IDNo='$EmployeeID'";
         $q1 = sqlsrv_query($conntest, $sql1);
         while ($row = sqlsrv_fetch_array($q1, SQLSRV_FETCH_ASSOC)) {
             $name = $row['Name'];
             $Department = $row['Department'];
...
<div class="card-body table-responsive " style="height: 100%;">
   <?php 
      ...
      $location="SELECT *,faulty_track.reference_no as ref, stock_summary.Status as ss_status from faulty_track inner join stock_summary on faulty_track.article_no=stock_summary.IDNo INNER join master_article ma on stock_summary.ArticleCode=ma.ArticleCode where faulty_track.token_no='$id' and faulty_track.status=1";
          ...
          $staff="SELECT Name,Imagepath FROM Staff Where IDNo='$emp'";
                     $stmt = sqlsrv_query($conntest,$staff);  
          ...
          if ($count%2!=0) 
             ...
             $aaaa.="<div class='direct-chat-msg'>
              ...
              <img class='direct-chat-img' src='".$BasURL.'Images/Staff/'.$Emp_Image."' alt='message user image'>";
...
<div class="col-lg-10">
<?php echo '<center><img src="'.$BasURL.'Images/Students/'.$img.'" height="100" width="100" class="img-thumnail"  style="border-radius:50%"/></center>';?>
...
<table class="table" border="1">
   <tr>
      ...
      <td rowspan="2"><?php echo '<center><img src="'.$BasURL.'Images/Students/'.$img.'" height="100%" width="100%" class="img-thumnail"/></center>';?></td>
   </tr>
   <tr>
      <td>Course: <?=$course?></td>
      <td>Batch: <?=$batch?></td>
...
$update_query=sqlsrv_query($conntest,$update1);
   ...
   {
       $concession_unit=$getconcessionsrundata1['concession_unit'];
   
   }
   $unitsConsumed=$unitsConsumed-$concession_unit;
   ...
   {   
       $ins="INSERT INTO meter_reading (article_no, location_id, current_owner, reading_date, current_reading, unit,unit_rate,amount,concession) VALUES ('$articleID','$locationID','$currentOwner','$date','$currentReading','$unitsConsumed','$unitRate','$bill','$concession_unit')";
       mysqli_query($conn,$ins);
    ...
    <table class="table" id="example">
        ...
        while($data=mysqli_fetch_array($res))
        {
         $Semester=$data['Semester'];
          $sqlSubject = "SELECT Course,SubjectName from MasterCourseStructure WHERE SubjectCode ='".$subjectCode."' and CourseID='".$courseId."'  order by SrNo DESC";
                    $resultSubject = sqlsrv_query($conntest,$sqlSubject);
                    if($rowSubject= sqlsrv_fetch_array($resultSubject, SQLSRV_FETCH_ASSOC) )
                    {
            ...
            <div class="card card-widget widget-user-2">
              ...
              <div class="widget-user-header bg-warning">
                <div class="widget-user-image">
                  <?php echo '<img src="'.$BasURL.'Images/Students/'.$img.'" height="100" width="100" class="img-circle elevation-2"  style="border-radius:50%"/>';?>
                </div>
                <!-- /.widget-user-image -->
            ...
            <!-- /.widget-user -->
              ...
              <div class="widget-user-header bg-warning">
                ...
                <div class="widget-user-image float-right">
                  <img class="img-circle elevation-2" width="50" height="50" style="border-radius:50%" src="<?=$BasURL.'Images/Staff/'.$empImg?>"  alt="User Avatar">   
                </div>
                <!-- /.widget-user-image -->
...
?>
   ...
   <td colspan="1">
      <button type="button"  onclick="exam_type_update(<?=$formid;?>);"  class="btn btn-primary"><i class="fa fa-check"></i></button></td>
 </tr>
...
// $img=$row['Snap'];
  ...
  <tr>
    ...
    <td>
      <div class="filter-container p-0 row">
                           <div class="filtr-item col-sm-2" data-category="1" data-sort="white sample">
                              <a href="<?=$BasURL;?>Images/Staff/<?php echo $IDNo;?>.jpg" data-toggle="lightbox" data-title="<?=$IDNo;?>">
                                 <img src="<?=$BasURL;?>Images/Staff/<?php echo $IDNo;?>.jpg" id="img_u"  height="50" width="50" class="img-circle elevation-2"  style="border-radius:50%" alt="image"/>
                              </a>
                           </div>
                        </div>
...
<br>
 ...
 <div class="col-lg-12 col-md-12" style="text-align:center;"><br>
                            ...
                            <?php // echo "http://erp.gku.ac.in:86/Images/Staff/".$univ_rollno.".jpg" ?>
                                 ...
                                 <img src="<?=$BasURL;?>Images/Staff/<?php echo $image;?>" id="img_u"  height="100" width="100" class="img-circle elevation-2"  style="border-radius:50%" alt="image"/>
...
$target_dir = "";
 include "connection/ftp-erp.php";
      ...
      $file_name = $IdNo."_".strtotime($date)."_".$string."_".basename($_FILES['imgage']['name']);
      $destdir = '/Images/Staff';
          ftp_chdir($conn_id, "/Images/Staff/") or die("Could not change directory");
          ftp_pasv($conn_id,true);
          ftp_put($conn_id, $file_name, $file_tmp, FTP_BINARY) or die("Could not upload to $ftp_server");
          ftp_close($conn_id);
           $insertExp="UPDATE Staff SET Imagepath='$file_name' where IDNo='$IdNo'";
...
?>  <div  class="table table-striped table-responsive">
    ...
    <tbody >
      <?php 
      $sql="SELECT * FROM Staff where Department='Class Four'";
$result = sqlsrv_query($conntest,$sql); 
while($row=sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC) )
{
$IDNo=$row['IDNo'];
      ...
      $total=count($array);
               ...
               { 
                  $Emp_Name='';
                             $Emp_Image='';
                             $emp_pic='';
                  $staff="SELECT Name,Imagepath FROM Staff Where IDNo='$array[$i]'";
                           $stmt = sqlsrv_query($conntest,$staff);  
                           while($row_staff = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC) )
                           {
                             $Emp_Name=$row_staff['Name'];
               ...
               ?>
               <li>
                  <img src="<?=$BasURL.'Images/Staff/'.$Emp_Image?>" alt="User Image" height="128px" style="height: 70px; width: 70px;">
...
{   
   ...
   ?> <ul class="users-list clearfix">
               ...
               { 
                             ...
                             $Emp_Image='';
                             $emp_pic='';
                  $staff="SELECT Name,Imagepath,IDNo FROM Staff Where IDNo='$array[$i]'";
                           $stmt = sqlsrv_query($conntest,$staff);  
                           while($row_staff = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC) )
                           {
               ...
               <li>
                  <a href="reports.php">
                     ...
                     {
                        ?>
                  <img src="<?=$BasURL.'Images/Staff/'.$Emp_Image?>" alt="<?=$Emp_Name?>" height="128px" style="height: 40px; width: 40px;">
                     <?php
                     }
                     else
                     {
 ...
 while($row = sqlsrv_fetch_array($result))  
      ...
      $name = $row['Name'];
       ...
       ?> 
         ...
         <td> 
 <button class="btn btn-success btn-xs" onclick="grant(<?=$row['RequestNo'];?>,<?=$empid;?>)">Grant</button>
...
<button class="btn btn-info btn-xs" onclick="view_movment_status(<?php echo $id;?>)" data-toggle="modal" data-target="#myExtraLargeModalLabel"><i class="fa fa-eye"></i>
         </button> 
...
<button class="btn btn-warning btn-xs"  ><i>Waiting</i>
         ...
         &nbsp;&nbsp;&nbsp;<button class="btn btn-warning btn-xs"  name='Check-in' onclick="cancel(<?php echo  $row['RequestNo'];?>)"><i class="fa fa-trash-alt"></i>
         </button> 
<?php }
...
while($row=sqlsrv_fetch_array($result)) 
 ...
 <button class="btn btn-danger btn-xs" onclick="checkout('<?=$id;?>')">Check-Out</button>
      ...
      <?php 
         ...
         else if($status=='check-out' AND $row['Purpose']=='Leave' AND $row['Leave']!='Full' ) {
         ?>
...
<?php 
  ...
  elseif($code=='304') 
   ...
   $College=$_POST['College'];
    ...
    $sql = "SELECT  Id,DepartmentFullName FROM MasterDepartment WHERE collegeId='$College' order by DepartmentFullName ASC";
 ...
 <?php   }
  ...
  elseif($code=='305') 
   ...
   $sql = "SELECT DISTINCT Course,CourseID,Duration FROM MasterCourseCodes WHERE CollegeID='$College' ANd DepartmentId='$Department' ANd (Status='1'  OR Status is NULL)  AND (CreditCardOpen!='1' OR CreditCardOpen is NULL)  AND  ((OpenDate>'$todaydate') OR (OpenDate is NULL))  order by Course ASC";
   $stmt = sqlsrv_query($conntest,$sql);  
   echo "<option value=''>Course</option>";
          while($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC) )
  ...
  elseif($code=='305.1') 
   ...
   $College=$_POST['College'];
   $Department=$_POST['department'];
     $sql = "SELECT DISTINCT Course,CourseID FROM MasterCourseCodes WHERE Batch>2024 and  CollegeID='$College' ANd DepartmentId='$Department' ANd (Status='1'  OR Status is NULL) AND (CreditCardOpen!='1' OR CreditCardOpen is NULL)   order by Course ASC";
   $stmt = sqlsrv_query($conntest,$sql);  
   echo "<option value=''>Course</option>";
          while($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC) )
...
?>
  ...
  <div class="card-body ">
    <div class="card-header ">
<center><h6>Apply Concession</h6></center>
...
<div class="row">
     <div class="col-lg-12">
     <label>Eployee ID<span class="text-danger">&nbsp;*</span></label>
     <input type="text" name="EmpID"   value="" onblur="empdatashow(this.value); fetch_leave_Balance(this.value)"   class="form-control">
    <span id="employee_name_show" style="color: red;">
    </span>
 </div>
    <br>
...
$query = "SELECT * FROM Staff Where Designation='Vice Chancellor' AND  JobStatus='1'";
     ...
     $Notification="INSERT INTO Notification (EmpID,SendBy,Subject, Discriptions, Page_link, DateTime, Status,Notification_type) VALUES ('$EmployeeID','$idno','Waiting for meeting','NA', 'visitor-admin.php','$timeStamp','0','1')";
...
elseif($code==344)
{
  ...
  $Notification="INSERT INTO Notification (EmpID,SendBy,Subject, Discriptions, Page_link, DateTime, Status,Notification_type)
...
$query = "SELECT * FROM Staff Where Designation='Vice Chancellor' AND  JobStatus='1'";
      ...
      $Notification="INSERT INTO Notification (EmpID,SendBy,Subject, Discriptions, Page_link, DateTime, Status,Notification_type) VALUES ('$Empid','$EmployeeID','$name waiting for meeting','NA', 'visitor-admin.php','$timeStamp','0','1')";
$result = sqlsrv_query($conntest,$Notification);
 sqlsrv_close($conntest);
...
$getdistri="Select Id from DDL_TheroyExamination where Value='$ecat'" ;
...
$emp_id=$location_row['OwerID'];
          ...
          $sqlas="SELECT Name,Department,Designation,CollegeName,Imagepath FROM Staff Where IDNo='$emp_id'";
...
$entrytime = date('H:i');
$status='draft';
$result = "INSERT into MovementRegister(EmpID,Purpose,LocationType,Reason,CheckOut,Status,Supervisor,Leave,RequestTime) values ('$EmployeeID','$purpose','$location','$remarks','$exittime','$status','$Recommend','$leavetype','$timeStamp')";
sqlsrv_query($conntest,$result);
$Notification="INSERT INTO Notification(EmpID, SendBy, Subject, Discriptions, Page_link, DateTime, Status,Notification_type) VALUES ('$Recommend', '$EmployeeID', 'Time out Request','$noti', 'movement-admin.php','$timeStamp','0','0')";
           sqlsrv_query($conntest,$Notification);
 sqlsrv_close($conntest);
 }
else if($code=='381')
{
 $id=$_POST['id'];
 $result ="Update MovementRegister set status='Self Canceled',CanceledTime='$timeStamp' where RequestNo='$id'";
sqlsrv_query($conntest,$result);
 sqlsrv_close($conntest);
}
else if($code=='382')
...
elseif($code==386) 
   {
    ...
    if($sub_data==2)
    {
      ...
      {
         $result = mysqli_query($conn_online,"SELECT * FROM online_payment where  status='success' AND remarks='4th Convocation' AND account_verification>'3'");
      }
      else
      {
...
<?php 
 ...
 elseif($code==387)
  ...
  //echo  $Pending = mysqli_fetch_assoc($result);
   ...
   $result = mysqli_query($conn_online,"SELECT count(*)  FROM online_payment where  status='success' AND remarks='4th Convocation' ANd account_verification>'0' ANd account_verification!='2' ");
  //echo  $Pending = mysqli_fetch_assoc($result);
...
<?php 
  ...
  //echo  $Pending = mysqli_fetch_assoc($result);
   ...
   $result = mysqli_query($conn_online,"SELECT count(*)  FROM online_payment where  status='success' AND remarks='4th Convocation' ANd account_verification='3'");
  //echo  $Pending = mysqli_fetch_assoc($result);
   $Rejeted = mysqli_fetch_row($result)[0];

   $result = mysqli_query($conn_online,"SELECT count(*)  FROM online_payment where  status='success' AND remarks='4th Convocation' ANd account_verification='4'");
  //echo  $Pending = mysqli_fetch_assoc($result);
   $ForwardToAccount = mysqli_fetch_row($result)[0];
    ...
    $counter = 1; 
      ...
      $acverify_date=$row['AccountVerificationDate'];
       ...
       $ac_reason=$row['AccountReason'];
         ...
         //$rerejectdate=$row=['EligibilityDate'];
       $rerejectdate=$row['EligibilityRejectDate'];
             }
  $sql = "SELECT  * FROM Admissions where UniRollNo='$roll_no'";
$stmt1 = sqlsrv_query($conntest,$sql);
        while($row6 = sqlsrv_fetch_array($stmt1, SQLSRV_FETCH_ASSOC) )
...
<tr>
 ...
 <p style="color: red;text-align: center;"> <b>Form Reject Detail(By Registration)</b></p>

        <h6>Rejected By  Registration Due to <b><u><?=$reg_reason;?></u></b>on: <?= $rerejectdate;?></td></h6>
</tr>
...
<td colspan="5"><p style="color: green;text-align: center; font-size: 16px;"> <b>Form Verification Detail(By Registration Branch)</b></p>
<textarea class=" form-control "name="" id="remarkReject"  ></textarea>
    <small id="error-reject-textarea" class='text-danger' style='display:none;'>Please enter
                                    a value minimum 5 characters.</small><br>
<center>
...
<tr>
 ...
 $result = mysqli_query($conn_online,"Update online_payment set account_verification='4',EligibilityBy='$EmployeeID',EligibilityDate='$timeStamp' where slip_no='$id' ");
