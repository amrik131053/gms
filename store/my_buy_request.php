...
$sql = '';
 ...
 $list_sql = "SELECT * FROM request_buy where emp_id='$emp_id' AND request_status='draft'  ORDER BY ID DESC ";
...
mysqli_fetch_array($result))  
      {   $i=0;
           $output .= '  
                <tr> 
                <input type="hidden" name="code" value='.'3.1'.'> 
          ...
          <td>'.$row["specification"].'</td>
            <td>'.$row["quantity"].'</td>
          <td>
            <a href="#" onclick="delete_dr_buy('.$row['id'].')"  class="fa fa-trash" data-target=".bs-delete-model-sm" data-toggle="tooltip"  data-placement="top" style="color:red" title="Delete"></a>
                  </td>
                </tr>  
           ';  
 ...
 $output .= '</tbody></table>'; 
    if(mysqli_num_rows($result) > 0)  
    { 
    $output .='<input type="submit" class="btn btn-warning" value="Submit for purchase"><br/><hr>';
    }
 $output .= '</form>
