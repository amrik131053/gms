...
if($this->PageNo() == 1){
    ...
    $this->MultiCell(80,3,'Authorised Signatory','0','C');
}
}
}
// $pdf = new FPDF('P');  // 
  ...
  $sql="SELECT *, MasterDepartment.Department as DepartmentName FROM Staff inner join MasterDepartment ON Staff.DepartmentId=MasterDepartment.Id where   IDNo='$empid'";
    ...
    while($row=sqlsrv_fetch_array($result))
    {
       $name= $row['Name'];
        ...
        $pdf-> Image('dist\img\idcardbg.png',0,81,53.98,4.5);
    ...
    $pdf->Write(3,'Emp.ID  :','0','L');
    $pdf->SetXY(1,$XSet+2);
    $pdf->Write(3,'Designation :','0','L');
    $pdf->SetXY(0.9,$XSet+8);
    $pdf->Write(3,'Department  :','0','L');
    $pdf->SetXY(0.9,$XSet+14);
    $pdf->Write(3,'Blood Group :','0','L');
...
elseif($strlen>23)
{
    ...
    $pdf->MultiCell(39,3,$row['DepartmentName'],'0','L');
      $pdf->SetXY(21,$XSet+14);
     $pdf->MultiCell(39,3,$row['BloodGroup'],'0','L');
    $pdf->SetXY(0,0);
   ...
   // $pdf->line(0,10.1,1000,10.1);
    ...
    $pdf->SetXY(0,57);
        ...
        $pdf->MultiCell(53.98,3,'This is property of Guru Kashi University','0','C');
   }
