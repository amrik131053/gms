<?php
 ...
 if ($row1 = sqlsrv_fetch_array($list_result1, SQLSRV_FETCH_ASSOC)) {
     ...
     while (
         $row_getResullt1 = sqlsrv_fetch_array(
             $list_getResullt1,
             SQLSRV_FETCH_ASSOC
         )
     ) {
   ...
   <th>	SUB{$count}	</th>
   <th>	SUB{$count}_TH_MAX	</th>
   <th>	SUB{$count}_PR_MAX	</th>
   <th>	SUB{$count}_CE_MAX	</th>
   <th>	SUB{$count}_TH_MRKS	</th>
   <th>	SUB{$count}_PR_MRKS	</th>
   <th>	SUB{$count}_CE_MRKS	</th>
   <th>	SUB{$count}_TOT	</th>
   <th>	SUB{$count}_STAUTS	</th>
   <th>	SUB{$count}_GRADE	</th>
   <th>	SUB{$count}_GRADE_POINTS	</th>
   <th>	SUB{$count}_CREDIT	</th>
   <th>	SUB{$count}_CREDIT_POINTS	</th>
   <th>	SUB{$count}_REMARKS	</th>
   <th>	SUB{$count}_CREDIT_ELIGIBILITY	</th>";
         $count++;
     }
 }
 ...
 while ($row = sqlsrv_fetch_array($list_result, SQLSRV_FETCH_ASSOC)) {
     ...
     while (
         $row_getResullt = sqlsrv_fetch_array(
             $list_getResullt,
             SQLSRV_FETCH_ASSOC
         )
     ) {
     ...
     for ($i = 0; $i < $c; $i++) {
         $exportstudy .= " 
                            ...
                            <td></td>
                            <td></td>
                            <td>{$SubjectGrade[$i]}</td>
                            <td>{$SubjectCredit[$i]}</td>
                            <td>{$NoOfCredit[$i]}</td>
                            <td>{$NoOfCredit[$i]}</td>
                            <td></td>
                            <td></td>
