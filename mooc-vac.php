...
$result1 = "SELECT  * FROM Admissions where  IDNo='$reg_id'";
      ...
      if($row = sqlsrv_fetch_array($stmt1, SQLSRV_FETCH_ASSOC) )
      {
       ...
       <tr>
         ...
         <td><?=$course;?></td>
         <td>
            <form action="mooccertificte.php"  method="POST" target="_blank" >
                <input type="hidden" value="<?=$row['IDNo'];?>" name="id"> <button class="btn btn-warning btn-xs">Download</button></form>
            </td>
...
{ 
 ...
 // $certificateid =$_POST['cert'];
  ...
  $sql1 = "INSERT INTO vacmooc(IDNo,VCourseName,Marks) VALUES ('$IDNo','$Vcoursename','$marks')";
   ...
   ?>
<script>alert('Successfully inserted Total: '+<?php echo $c ?>);window.location.href = 'mooc-vac.php';</script>
   <?php 
}
