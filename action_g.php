...
sqlsrv_query($conntest,$updateLeaveBalance);
    ...
    ?>
        ...
        while($ifLEaveRow=sqlsrv_fetch_array($ifLeaveCheckWhenAuthRun))
        {
            ...
            {
                $getAllleaves="SELECT  *,LeaveTypes.Name as LeaveTypeName,Staff.Name as StaffName,ApplyLeaveGKU.Id as LeaveID FROM Staff inner join ApplyLeaveGKU ON Staff.IDNo=ApplyLeaveGKU.StaffId  inner join LeaveTypes ON LeaveTypes.Id=ApplyLeaveGKU.LeaveTypeId  where YEAR(StartDate)>='2023'  ANd  LeaveSanctionAuthority='$EmployeeID' and ApplyLeaveGKU.Status='Pending To Authority'  and ApplyLeaveGKU.Status!='Reject' and  StaffId='".$ifLEaveRow['IDNo']."' order by  ApplyLeaveGKU.Id DESC ";
            }
            ...
            {
              echo    $getAllleaves="SELECT  *,LeaveTypes.Name as LeaveTypeName,Staff.Name as StaffName,ApplyLeaveGKU.Id as LeaveID FROM Staff inner join ApplyLeaveGKU ON Staff.IDNo=ApplyLeaveGKU.StaffId  inner join LeaveTypes ON LeaveTypes.Id=ApplyLeaveGKU.LeaveTypeId  where YEAR(StartDate)>='2023' AND   StaffId='".$ifLEaveRow['IDNo']."' and  ApplyLeaveGKU.Status='Pending To VC' and  ApplyLeaveGKU.Status!='Approved' and ApplyLeaveGKU.Status!='Reject' order by  ApplyLeaveGKU.Id DESC "; 
...
elseif($code==235)
{
    ...
    // ECHO $timeStamp;
       $remarks =str_replace("'",'',$_POST['remarks']);
    $updateLeaveAcrodingToAction="UPDATE  ApplyLeaveGKU  SET Status='Pending to Authority',SanctionRemarks='$remarks',SanctionApproveDate='$timeStamp' WHERE Id='$id'";
    $updateLeaveAcrodingToActionRun=sqlsrv_query($conntest,$updateLeaveAcrodingToAction);
    $getAllleaves="SELECT *,LeaveTypes.Name as LeaveTypeName,Staff.Name as StaffName,ApplyLeaveGKU.Id as LeaveID FROM Staff inner join ApplyLeaveGKU ON Staff.IDNo=ApplyLeaveGKU.StaffId  inner join LeaveTypes ON LeaveTypes.Id=ApplyLeaveGKU.LeaveTypeId  where  ApplyLeaveGKU.Id='$id' "; 
    $getAllleavesRun=sqlsrv_query($conntest,$getAllleaves);
    if($row=sqlsrv_fetch_array($getAllleavesRun,SQLSRV_FETCH_ASSOC))
...
elseif($code==237)
{
    ...
    $updateLeaveAcrodingToAction="UPDATE  ApplyLeaveGKU  SET Status='Reject',SanctionRemarks='$remarks',SanctionApproveDate='$timeStamp' WHERE Id='$id'";
    $updateLeaveAcrodingToActionRun=sqlsrv_query($conntest,$updateLeaveAcrodingToAction);
    $getAllleaves="SELECT *,LeaveTypes.Name as LeaveTypeName,Staff.Name as StaffName,ApplyLeaveGKU.Id as LeaveID FROM Staff inner join ApplyLeaveGKU ON Staff.IDNo=ApplyLeaveGKU.StaffId  inner join LeaveTypes ON LeaveTypes.Id=ApplyLeaveGKU.LeaveTypeId  where  ApplyLeaveGKU.Id='$id' "; 
    $getAllleavesRun=sqlsrv_query($conntest,$getAllleaves);
    if($row=sqlsrv_fetch_array($getAllleavesRun,SQLSRV_FETCH_ASSOC))
...
elseif($code==240)
{
    ...
    $remarks =str_replace("'",'',$_POST['remarks']);
     $updateLeaveAcrodingToAction="UPDATE  ApplyLeaveGKU  SET Status='Reject',HRRemarks='$remarks',HRApprovedate='$timeStamp' WHERE Id='$id'";
    $updateLeaveAcrodingToActionRun=sqlsrv_query($conntest,$updateLeaveAcrodingToAction);
    $getAllleaves="SELECT *,LeaveTypes.Name as LeaveTypeName,Staff.Name as StaffName,ApplyLeaveGKU.Id as LeaveID FROM Staff inner join ApplyLeaveGKU ON Staff.IDNo=ApplyLeaveGKU.StaffId  inner join LeaveTypes ON LeaveTypes.Id=ApplyLeaveGKU.LeaveTypeId  where  ApplyLeaveGKU.Id='$id' "; 
    $getAllleavesRun=sqlsrv_query($conntest,$getAllleaves);
    if($row=sqlsrv_fetch_array($getAllleavesRun,SQLSRV_FETCH_ASSOC))
...
$update_query=sqlsrv_query($conntest,$update1);
   ...
   else if($code==294)
   {
    ...
    if ($Examination == '') {
        ...
        $list_sql .= " AND  ExamForm.Status = '5'";
   ...
   else if($code==295)
   {
    ...
    if ($Examination == '') {
        ...
        $list_sql .= " AND  ExamForm.Status = '7'";
    $getDefaultMenuRun = sqlsrv_query($conntest, $list_sql);
    if ($row = sqlsrv_fetch_array($getDefaultMenuRun, SQLSRV_FETCH_ASSOC)) {
        echo $row['Count'];
    } 
...
if ($insert_record_run === false) {
   ...
   elseif($code==313)
   {
     ...
     foreach($ids as $key => $id)
     {
          $getDefalutMenu="DELETE  FROM  ValueAddedCertificate Where ID='$id'";
           $getDefalutMenuRun=sqlsrv_query($conntest,$getDefalutMenu); 
...
$update_query=sqlsrv_query($conntest,$update1);
   ...
   elseif($code==314)
   {
     ...
     foreach($ids as $key => $id)
     {
          $getDefalutMenu="DELETE  FROM  VACertificateSignature Where Id='$id'";
           $getDefalutMenuRun=sqlsrv_query($conntest,$getDefalutMenu); 
...
$update_query=sqlsrv_query($conntest,$update1);
   ...
   elseif($code==315)
   {
     ...
     $ids=$_POST['certificateID'];
          ...
          $getDefalutMenu="DELETE  FROM  VACertificateSignature Where Id='$ids'";
           $getDefalutMenuRun=sqlsrv_query($conntest,$getDefalutMenu); 
 ...
 echo json_encode($count);
...
else if($code==324)
   {
    ...
    $list_sql = "SELECT COUNT(*) as Count
                 FROM ExamForm
                 INNER JOIN Admissions ON ExamForm.IDNo = Admissions.IDNo 
                 WHERE ExamForm.Status = '4' AND Admissions.Status='1'";
    if ($College !== '') {
        $list_sql .= " AND ExamForm.CollegeID = '$College'";
    }
...
elseif($Status==8)
             ...
             ?>
             <tr style="background-color:<?=$trColor;?>;font-size:14px;">
             <td><?php if($Status=='4'){ ?><input type="checkbox" class="checkbox v_check" value="<?= $row['ID'];?>"><?php }?></td>
             <td><?= $count++;?></td>
...
elseif($code==326)
{
  ...
  foreach($ids as $key => $id)
  {
       $getDefalutMenu="UPDATE  ExamForm  SET AccountantVerificationDate='$timeStampS',Status='5' Where ID='$id'";
        // $getDefalutMenuRun=sqlsrv_query($conntest,$getDefalutMenu); 
     $getStudentID="SELECT IDNo FROM ExamForm WHERE ID='$id'";
     $getStudentIDRun=sqlsrv_query($conntest,$getStudentID);
     if ($row = sqlsrv_fetch_array($getStudentIDRun, SQLSRV_FETCH_ASSOC)) {
...
$update_query=sqlsrv_query($conntest,$update1);
   ...
   else if($code==330)
   {
    ...
    $list_sql = "SELECT COUNT(*) as Count
                 FROM ExamForm
                 INNER JOIN Admissions ON ExamForm.IDNo = Admissions.IDNo 
                 WHERE ExamForm.Status = '6' AND Admissions.Status='1'";
    if ($College !== '') {
        $list_sql .= " AND ExamForm.CollegeID = '$College'";
    }
   ...
   else if($code==331)
   {
    ...
    $list_sql = "SELECT COUNT(*) as Count
                 FROM ExamForm
                 INNER JOIN Admissions ON ExamForm.IDNo = Admissions.IDNo 
                 WHERE ExamForm.Status >= '5' And ExamForm.Status!= '6' AND Admissions.Status='1' ";
    if ($College !== '') {
        $list_sql .= " AND ExamForm.CollegeID = '$College'";
    }
   ...
   elseif($code==332)
   {
    ...
    {
        echo $_POST['Examination'];
    }
    else
    {
        echo $CurrentExamination;
    }
...
<table class="table  table-head-fixed" style="font-size:12px;">
    <thead>
   <tr>
<th>Batch</th>
<th>Semester</th>
...
if($_POST['Examination']!='')
{
    ...
    { 
     ...
     $getAdmissionsTotal="SELECT Distinct IDNo FROM Admissions  WHERE Batch='".$rowCourseName['Batch']."' and CourseID='$CourseID' and Status='1' ";
    $getAdmissionsTotal_run=sqlsrv_query($conntest,$getAdmissionsTotal,array(), array( "Scrollable" => SQLSRV_CURSOR_KEYSET ));
    $AdmissionsTotal=sqlsrv_num_rows($getAdmissionsTotal_run);

    $examFormAccepted12 = "SELECT IDNo FROM Admissions WHERE Admissions.Status='1' AND IDNo NOT IN 
    (SELECT IDNo FROM ExamForm WHERE SemesterId='" . $rowCourseName['SemesterId'] . "' AND CourseID='$CourseID' AND Batch='".$rowCourseName['Batch']."')  AND CourseID='$CourseID' AND Batch='".$rowCourseName['Batch']."'";
    ...
    $Accepeted=sqlsrv_num_rows($getleftTotal_run);
     ?>
    <tr>
       <td><?=$rowCourseName['Batch'];?></td>
       <td><?=$rowCourseName['SemesterId'];?></td>
       <td><?=$Examination;?></td>
       <td><button class="btn btn-success btn-xs"  onclick="exportTotalAdm(<?=$rowCourseName['CollegeID'];?>,<?=$CourseID;?>,<?=$rowCourseName['Batch'];?>);"><?=$AdmissionsTotal;?></button></td>
       <td><button class="btn btn-success btn-xs" onclick="exportTotalExamFormNotApplied(<?=$rowCourseName['SemesterId'];?>,<?=$CourseID;?>,<?=$rowCourseName['Batch'];?>);"><?=$countExamFormNotApply;?></button></td>
       <td><b class="text-primary"><?=$Total;?></td>
       <td><b class="text-primary"><?=$Registration.'</b>/<b class="text-danger">'.$RegistrationReject.'</b>';?></td>
       <td><b class="text-primary"><?=$Department.'</b>/<b class="text-danger">'.$DepartmentReject.'</b>';?></td>
       <td><b class="text-primary"><?=$Account.'</b>/<b class="text-danger">'.$AccountReject.'</b>';?></td>
       <td><b class="text-primary"><?=$Examination1.'</b>/<b class="text-danger">'.$ExaminationReject.'</b>';?></td>
       <td><b class="text-success"><?=$Accepeted;?></b></td>
       <td>  <button type="button" class="btn btn-tool" 
                                                onclick="exportTotalScordingToBatchSumy(<?=$CourseID;?>,<?=$rowCourseName['Batch'];?>,<?=$rowCourseName['SemesterId'];?>);" style="float:right;padding:15px;">
...
<?php
   ...
   else if($code==334)
   {
    ...
    if ($Examination == '') {
        ...
        $list_sql .= " AND  (ExamForm.Status = '0' or ExamForm.Status = '1')";
   ...
   else if($code==335)
   {
    ...
    if ($Examination == '') {
        ...
        $list_sql .= " AND  (ExamForm.Status = '2' or ExamForm.Status = '3')";
   ...
   else if($code==336)
   {
    ...
    if ($Examination == '') {
        ...
        $list_sql .= " AND  ExamForm.Status = '8'";
